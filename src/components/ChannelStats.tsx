...
function getChannelAge() {
  ...
  const hadBirthday =
    ...
    (now.getMonth() === CHANNEL_CREATED.getMonth() &&
      now.getDate() >= CHANNEL_CREATED.getDate());
...
function getNextAnniversary() {
  ...
  let next = new Date(
    thisYear,
    CHANNEL_CREATED.getMonth(),
    CHANNEL_CREATED.getDate(),
  );
  if (next <= now) {
    next = new Date(
      thisYear + 1,
...
function CountdownCard({
  ...
  const d = countdown ?? { days: 0, hours: 0, minutes: 0, seconds: 0 };
      ...
      <div className="channel-stats-grid-wrapper">
        <div className="channel-stats-grid">
          ...
          <StatCard
            ...
            isInView={isInView}
          />
          <CountdownCard delay={0.4} isInView={isInView} />
        </div>
      </div>
    </section>
  );
