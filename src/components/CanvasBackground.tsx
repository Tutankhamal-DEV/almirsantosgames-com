...
const soccerState = {
    ...
    scoreA: 0, scoreB: 0, resetTimer: 0, kickCD: 0,
...
function renderStandard(
    ...
    if (mouse.x >= 0) {
        ...
        for (let i = -r; i <= r; i++) {
            for (let j = -r; j <= r; j++) {
                ...
                grid[idx] = Math.min(grid[idx] + Math.max(0, 1 - dist / (r + 1)) * 0.6, 1);
            }
        }
    }
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            ...
            if (Math.random() > 0.997) grid[idx] = Math.min(grid[idx] + 0.3 + Math.random() * 0.4, 1);
        }
    }
...
function drawTileShape(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, shape: number) {
    const half = size / 2, cx = x + half, cy = y + half, s = half - 1;
    ctx.beginPath();
    switch (shape) {
        case 0: ctx.rect(x + 1, y + 1, size - 2, size - 2); break;
        case 1: ctx.moveTo(cx, cy - s); ctx.lineTo(cx + s, cy); ctx.lineTo(cx, cy + s); ctx.lineTo(cx - s, cy); ctx.closePath(); break;
        case 2: {
            const t = s * 0.35;
            ctx.moveTo(cx - t, cy - s); ctx.lineTo(cx + t, cy - s); ctx.lineTo(cx + t, cy - t);
            ctx.lineTo(cx + s, cy - t); ctx.lineTo(cx + s, cy + t); ctx.lineTo(cx + t, cy + t);
            ctx.lineTo(cx + t, cy + s); ctx.lineTo(cx - t, cy + s); ctx.lineTo(cx - t, cy + t);
            ctx.lineTo(cx - s, cy + t); ctx.lineTo(cx - s, cy - t); ctx.lineTo(cx - t, cy - t);
            ctx.closePath(); break;
        }
        case 3: ctx.arc(cx, cy, s * 0.6, 0, Math.PI * 2); break;
    }
    ctx.fill();
}
...
// ══════════════════════════════════════════
function renderPlatformer(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    ...
    for (let i = 0; i < 18; i++) {
        const cycle = w * 2;
        const lx = ((i * w * 0.13 + w * 0.05 + scrollT * 0.04) % cycle + cycle) % cycle - w * 0.25;
    ...
    for (let slot = farStart; slot < farStart + farVisible; slot++) {
        ...
        ctx.beginPath();
        ctx.arc(tx + trunkW / 2, h * 0.08 + Math.floor(h1 * 2) * h * 0.04, canR, 0, Math.PI * 2);
        ctx.fill();
    }
    ...
    for (let slot = midStart; slot < midStart + midVisible; slot++) {
        ...
        ctx.fillRect(tx + trunkW - w * 0.005, groundY0 - h * 0.015, w * 0.02, h * 0.035);
        ctx.fillStyle = '#143a14';
        const fR = w * 0.07 + Math.floor(h0 * 3) * w * 0.02;
        ctx.beginPath();
        ctx.arc(tx + trunkW / 2, h * 0.05 + Math.floor(h1 * 2) * h * 0.06, fR, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#1a4a1a';
        ctx.beginPath();
        ctx.arc(tx + trunkW / 2 - fR * 0.3, h * 0.1 + Math.floor(h1 * 2) * h * 0.04, fR * 0.7, 0, Math.PI * 2);
        ctx.fill();
    }
    ...
    for (let slot = nearStart; slot < nearStart + nearVisible; slot++) {
        ...
        if (seed === 1 || seed === 5) {
            ...
            ctx.beginPath();
            ctx.arc(sx + nearSpacing * 0.5 - bR * 0.5, bushY - bR * 0.4, bR * 0.75, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#347a2e';
            ctx.beginPath();
            ctx.arc(sx + nearSpacing * 0.5 + bR * 0.6, bushY - bR * 0.8, bR * 0.8, 0, Math.PI * 2);
            ctx.fill();
        }
    ...
    for (let slot = vineStart; slot < vineStart + vineVisible; slot++) {
        ...
        ctx.beginPath();
        ctx.ellipse(vx + sway * 0.5, vineLen, w * 0.006, h * 0.02, 0.3, 0, Math.PI * 2);
        ctx.fill();
    }
    ...
    for (let x = 0; x < w; x += w * 0.012) {
        const leafY = h * 0.12 + Math.sin(x * 0.06 + t * 0.3) * h * 0.02 + Math.sin(x * 0.12) * h * 0.012;
    ...
    for (let f = 0; f < 20; f++) {
        ...
        const fy = h * 0.12 + (f * 97) % (h * 0.55) + Math.cos(t * 2 + f * 2) * h * 0.02;
...
// ══════════════════════════════════════════
function renderSpaceship(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    // ── DEEP SPACE GRADIENT ──
    ...
    for (let layer = 0; layer < 4; layer++) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const sy = ((sHash(i * 311 + layer * 7919) * h + t * speed) % (h + 10)) - 5;
            const sz = Math.max(1, Math.floor(sHash(i * 431 + layer * 137) * maxSize) + 1);
            const twinkle = Math.sin(t * (2 + layer) + i * 0.7) * 0.2 + 0.4 + layer * 0.12;
            const hue = sHash(i * 97 + layer * 11) > 0.7 ? `rgba(180,200,255,${twinkle})` :
                sHash(i * 97 + layer * 11) > 0.4 ? `rgba(255,220,180,${twinkle})` :
    ...
    for (let i = 0; i < 5; i++) {
        ...
        grad.addColorStop(0.5, `rgba(${r * 0.6},${g * 0.5},${b * 0.7},${pulse * 0.6})`);
        grad.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = grad;
    ...
    const shipX = w * 0.5 + Math.sin(t * 0.7) * w * 0.15 + Math.sin(t * 1.3) * w * 0.08;
    const shipY = h * 0.72 + Math.sin(t * 0.5) * h * 0.04 + Math.cos(t * 1.1) * h * 0.02;
    const shipTilt = Math.cos(t * 0.7) * 0.15;

    // Engine trail
    for (let p = 0; p < 12; p++) {
        const trailX = shipX - Math.sin(t * 0.7 - p * 0.08) * w * 0.15 - Math.sin(t * 1.3 - p * 0.08) * w * 0.08;
    ...
    for (let b = 0; b < 8; b++) {
        ...
        const lx = w * 0.5 + Math.sin(fireTime * 0.7) * w * 0.15 + Math.sin(fireTime * 1.3) * w * 0.08;
    ...
    for (let e = 0; e < 10; e++) {
        ...
        if (formation === 0) {
            // Zigzag descent
            ex = w * 0.2 + sHash(e * 173) * w * 0.6 + Math.sin(t * 1.5 + ePhase) * w * 0.1;
            ey = ((t * 45 + sHash(e * 311) * h * 3) % (h * 1.2)) - h * 0.15;
        } else if (formation === 1) {
            // Circle patrol
            const circleR = w * 0.15 + sHash(e * 199) * w * 0.1;
            const circleSpeed = 0.5 + sHash(e * 277) * 0.5;
            ex = w * (0.3 + sHash(e * 137) * 0.4) + Math.cos(t * circleSpeed + ePhase) * circleR;
            ey = h * 0.15 + sHash(e * 353) * h * 0.35 + Math.sin(t * circleSpeed + ePhase) * circleR * 0.4;
        } else {
            // Sweep across
            ex = ((t * 60 + sHash(e * 509) * w * 4) % (w * 1.4)) - w * 0.2;
            ey = h * 0.1 + sHash(e * 613) * h * 0.4 + Math.sin(t * 2 + ePhase) * h * 0.05;
        }
        ...
        if (e % 3 === 0) {
            ...
            if (shootPhase < 0.5) {
                ...
                const bGlow = ctx.createRadialGradient(ex, bulletY + 4, 0, ex, bulletY + 4, 6);
    ...
    for (let x = 0; x < 6; x++) {
        ...
        if (progress < 0.2) {
            ...
            const flash = ctx.createRadialGradient(explX, explY, 0, explX, explY, flashR);
    ...
    for (let a = 0; a < 7; a++) {
        const ax = ((sHash(a * 211) * w * 3 + t * (15 + sHash(a * 137) * 10)) % (w + 60)) - 30;
        const ay = ((sHash(a * 349) * h * 3 + t * (8 + sHash(a * 193) * 6)) % (h + 60)) - 30;
...
// ══════════════════════════════════════════
function renderRacing(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    const dt = 0.016;
    ...
    if (RS.playerLaneTimer <= 0) {
        ...
        RS.playerLaneTarget = available[Math.floor(Math.random() * available.length)];
    ...
    const moonHalo = ctx.createRadialGradient(moonX, moonY, moonR * 0.5, moonX, moonY, moonR * 5);
    ...
    const moonGrad = ctx.createRadialGradient(moonX - moonR * 0.25, moonY - moonR * 0.2, moonR * 0.1, moonX, moonY, moonR);
    ...
    ctx.beginPath(); ctx.arc(moonX + moonR * 0.2, moonY - moonR * 0.15, moonR * 0.18, 0, Math.PI * 2); ctx.fill();
    ...
    for (let s = 0; s < 3; s++) {
        ...
        if (age < 0.6) {
            ...
            const grad = ctx.createLinearGradient(headX, headY, headX - tailLen * 0.7, headY - tailLen * 0.35);
    ...
    const farBuildings = [
        { x: 0, w: 60, h: 110 }, { x: 65, w: 45, h: 80 }, { x: 115, w: 70, h: 140 },
        { x: 190, w: 50, h: 95 }, { x: 245, w: 80, h: 125 }, { x: 330, w: 55, h: 70 },
        { x: 390, w: 65, h: 155 }, { x: 460, w: 45, h: 85 }, { x: 510, w: 75, h: 120 },
        { x: 590, w: 55, h: 100 }, { x: 650, w: 60, h: 130 }, { x: 715, w: 50, h: 75 },
        { x: 770, w: 70, h: 145 }, { x: 845, w: 45, h: 90 }, { x: 895, w: 65, h: 110 },
        { x: 965, w: 55, h: 80 }, { x: 1025, w: 60, h: 135 },
    ];
    ...
    for (let copy = 0; copy * 1100 < farCycle + 1100; copy++) {
        for (const b of farBuildings) farBuildingsExpanded.push({ x: b.x + copy * 1100, w: b.w, h: b.h });
    }
    for (const b of farBuildingsExpanded) {
        const bx = ((b.x - cityScroll * 0.3) % farCycle + farCycle) % farCycle - 40;
    ...
    const nearBuildings = [
        { x: 0, w: 50, h: 90 }, { x: 55, w: 70, h: 130 }, { x: 130, w: 40, h: 75 },
        { x: 175, w: 65, h: 160 }, { x: 245, w: 55, h: 100 }, { x: 305, w: 50, h: 85 },
        { x: 360, w: 75, h: 145 }, { x: 440, w: 45, h: 65 }, { x: 490, w: 60, h: 120 },
        { x: 555, w: 50, h: 95 }, { x: 610, w: 70, h: 150 }, { x: 685, w: 55, h: 80 },
        { x: 745, w: 65, h: 110 }, { x: 815, w: 45, h: 135 }, { x: 865, w: 60, h: 70 },
        { x: 930, w: 75, h: 140 }, { x: 1010, w: 50, h: 90 },
    ];
    ...
    for (let copy = 0; copy * 1080 < nearCycle + 1080; copy++) {
        for (const b of nearBuildings) nearBuildingsExpanded.push({ x: b.x + copy * 1080, w: b.w, h: b.h });
    }
    const neonColors = ['255,50,120', '0,220,255', '80,255,80', '255,180,0', '180,80,255'];
    let nbIdx = 0;
    for (const b of nearBuildingsExpanded) {
        const bx = ((b.x - cityScroll * 0.6) % nearCycle + nearCycle) % nearCycle - 40;
        if (bx < -b.w || bx > w + 40) { nbIdx++; continue; }
        const bh = b.h * 0.7;
        ...
        for (let wy = by + 6; wy < horizonY - 5; wy += 9) {
            for (let wx = bx + 5; wx < bx + b.w - 5; wx += 11) {
                ...
                if (lit) {
                    ...
                    const hue = ((wx + wy) % 3 === 0) ? '255,200,60' : ((wx + wy) % 3 === 1) ? '255,180,40' : '220,200,80';
        ...
        if (nbIdx % 3 === 0 && b.w > 45) {
            ...
            const na = np + nf;
            const sw = b.w * 0.6, sH = 6;
            const sx = bx + (b.w - sw) / 2, sy = by + bh * 0.25;
            const ng = ctx.createRadialGradient(sx + sw / 2, sy + sH / 2, 2, sx + sw / 2, sy + sH / 2, sw * 0.7);
    ...
    const cg = ctx.createRadialGradient(w * 0.5, horizonY, w * 0.05, w * 0.5, horizonY, w * 0.5);
    ...
    const getCenterX = (progress: number) => {
        return baseCenterX + Math.sin(progress * Math.PI * 0.5) * RS.curveOffset * 0.3;
    };
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const poleH = eased * 28, poleW = Math.max(1, eased * 3), bulbR = Math.max(1, eased * 3);
        for (const lx of [lx1, lx2]) {
            ctx.fillStyle = '#555';
            ctx.fillRect(lx, lampY - poleH, poleW, poleH);
            const armDir = lx < cx ? 1 : -1;
            ctx.fillRect(lx, lampY - poleH, armDir * eased * 10, Math.max(1, eased * 2));
            ...
            const pool = ctx.createRadialGradient(poolX, lampY, 0, poolX, lampY, poolR);
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const postH = eased * 18, postW = Math.max(1, eased * 4);
    ...
    for (const opp of sortedOpps) {
        ...
        // Tail light glow
        if (scale > 0.2) {
            const tg = ctx.createRadialGradient(carX, objY + 12 * s, 0, carX, objY + 12 * s, 18 * s);
    ...
    const hlPool = ctx.createRadialGradient(playerX, hlPoolY, 5, playerX, hlPoolY, Math.max(hlPoolRX, hlPoolRY));
    ...
    // Subtle light streak
    const strG = ctx.createLinearGradient(playerX, playerY - 14, playerX, hlPoolY);
    ...
    // ── RETRO HUD ──
    const hudSpeed = Math.floor(160 + RS.speedPulse * 50 + Math.sin(t * 2.3) * 15);
...
function renderSports(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    ...
    for (let i = 0; i < 10; i++) { ctx.fillStyle = i % 2 === 0 ? '#1a5a1a' : '#1d6a1d'; ctx.fillRect(0, i * h / 10, w, h / 10); }
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.6)'; ctx.beginPath(); ctx.arc(w / 2, h / 2, 4, 0, Math.PI * 2); ctx.fill();
    ctx.strokeRect(M, h / 2 - goalH / 2, goalW, goalH);
    ctx.strokeRect(w - M - goalW, h / 2 - goalH / 2, goalW, goalH);
    ctx.beginPath(); ctx.arc(M + goalW, h / 2, goalH * 0.35, -0.5, 0.5); ctx.stroke();
    ctx.beginPath(); ctx.arc(w - M - goalW, h / 2, goalH * 0.35, Math.PI - 0.5, Math.PI + 0.5); ctx.stroke();
    ...
    // Wall bounces
    if (B.y < M + 5) { B.y = M + 5; B.vy = Math.abs(B.vy) * 0.7; }
    if (B.y > h - M - 5) { B.y = h - M - 5; B.vy = -Math.abs(B.vy) * 0.7; }
    const inGoalY = B.y > h / 2 - goalH / 2 && B.y < h / 2 + goalH / 2;
    if (B.x < M + 5 && inGoalY) { soccerState.scoreB++; soccerState.resetTimer = 1.2; B.vx = 0; B.vy = 0; }
    else if (B.x > w - M - 5 && inGoalY) { soccerState.scoreA++; soccerState.resetTimer = 1.2; B.vx = 0; B.vy = 0; }
    if (B.x < M + 5) { B.x = M + 5; B.vx = Math.abs(B.vx) * 0.7; }
    ...
    for (let i = 0; i < 22; i++) {
        ...
        if (p.team === 0 && d < runAD) { runA = i; runAD = d; }
        if (p.team === 1 && d < runBD) { runB = i; runBD = d; }
    }
    ...
    for (let i = 0; i < 22; i++) {
        ...
        if (ballDist < 11 && soccerState.kickCD <= 0 && soccerState.resetTimer <= 0) {
            ...
            if (action < 0.4) {
                ...
                const sdx = gx - B.x, sdy = gy - B.y;
                const sl = Math.hypot(sdx, sdy) || 1;
                const pw = 250 + Math.random() * 100;
                B.vx = (sdx / sl) * pw; B.vy = (sdy / sl) * pw * 0.4;
            } else if (action < 0.75) {
                // Pass (angled forward)
                const ang = dir * (0.2 + Math.random() * 0.8) + (Math.random() - 0.5) * 1.2;
                const pw = 100 + Math.random() * 80;
                B.vx = Math.cos(ang) * pw; B.vy = Math.sin(ang) * pw;
            } else {
                // Dribble (short kick)
                const ang = (Math.random() - 0.5) * Math.PI * 0.6 + (dir > 0 ? 0 : Math.PI);
    ...
    ctx.beginPath(); ctx.ellipse(B.x + 3, B.y + 4, 6, 3, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff';
    ...
    ctx.fillStyle = '#dc2626'; ctx.font = '18px "Jersey 10", monospace';
    ctx.textAlign = 'right'; ctx.fillText(String(soccerState.scoreA), w / 2 - 12, 29);
    ctx.fillStyle = '#fff'; ctx.textAlign = 'center'; ctx.fillText('×', w / 2, 28);
    ctx.fillStyle = '#5588dd'; ctx.textAlign = 'left'; ctx.fillText(String(soccerState.scoreB), w / 2 + 12, 29);
    ctx.textAlign = 'start';
}
...
function renderFPS(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    ...
    const sunBody = ctx.createLinearGradient(sunX, sunY - sunR, sunX, sunY + sunR);
    ...
    const farMountains = [0, 0.08, 0.18, 0.28, 0.35, 0.45, 0.52, 0.6, 0.7, 0.78, 0.88, 0.95, 1.0];
    const farHeights = [0.12, 0.22, 0.14, 0.30, 0.18, 0.35, 0.25, 0.38, 0.20, 0.32, 0.15, 0.25, 0.10];
    ...
    ctx.beginPath();
    ctx.moveTo(0, horizon);
    const nearMountains = [0, 0.1, 0.2, 0.32, 0.42, 0.55, 0.65, 0.75, 0.85, 0.92, 1.0];
    const nearHeights = [0.06, 0.14, 0.08, 0.18, 0.10, 0.06, 0.15, 0.22, 0.12, 0.08, 0.05];
    ...
    const reflGrad = ctx.createRadialGradient(sunX, horizon + h * 0.05, 0, sunX, horizon + h * 0.1, w * 0.3);
    ...
    // ── Edge vignette ──
    const vig = ctx.createRadialGradient(vanishX, horizon, w * 0.3, vanishX, horizon, w * 0.75);
...
function drawVaporPalm(ctx: CanvasRenderingContext2D, x: number, groundY: number, spread: number, height: number, lean: number, t: number) {
    ...
    ctx.quadraticCurveTo(x + lean * height * 0.4, groundY - height * 0.5, topX + sway, topY);
    ctx.stroke();
    ...
    for (let f = 0; f < frondCount; f++) {
        ...
        ctx.quadraticCurveTo(cpx + spread * 0.02, cpy + spread * 0.03, topX + sway, topY);
        ctx.fill();
    }
}
...
export default function CanvasBackground() {
    ...
    const getScene = useCallback((): Scene => SCENES[sceneIndexRef.current % SCENES.length], []);
    ...
    useEffect(() => {
        ...
        const resize = () => {
            ...
            for (let i = 0; i < newShapes.length; i++) newShapes[i] = Math.floor(Math.random() * 4);
        ...
        const onML = () => { mouseRef.current = { x: -1, y: -1 }; };
        ...
        const render = () => {
            ...
            switch (scene) {
                case 'standard':
                    renderStandard(ctx, w, h, timeRef.current, gridRef.current!, shapeGridRef.current!, dimensionsRef.current.cols, dimensionsRef.current.rows, mouseRef.current);
                    break;
                case 'platformer':
                    ctx.clearRect(0, 0, w, h);
