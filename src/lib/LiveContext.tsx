...
const LiveContext = createContext<LiveStatus>({ isLive: false, liveVideoId: null });
...
async function checkLiveStatus(): Promise<LiveStatus> {
    try {
        ...
        const isLiveStream = title.toLowerCase().includes('live') ||
...
export function LiveProvider({ children }: { children: ReactNode }) {
    const [status, setStatus] = useState<LiveStatus>({ isLive: false, liveVideoId: null });
    ...
    return (
        <LiveContext.Provider value={status}>
            {children}
